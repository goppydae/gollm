...
// expandAttachments replaces @path tokens in prompt with file contents wrapped
// in <file> tags. Tokens that cannot be read are left as-is.
func expandAttachments(prompt string) string {
	...
	rem := prompt
	for {
		at := strings.IndexByte(rem, '@')
		if at < 0 {
			b.WriteString(rem)
			break
		}
		b.WriteString(rem[:at])
		rem = rem[at+1:]
